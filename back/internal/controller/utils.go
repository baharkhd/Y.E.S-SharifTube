...
func CastDBExceptionToGQLException(err error) error {
	if err != nil {
		...
		case *database.EmptyFields:
			return model.EmptyFieldsException{Message: err.Error()}
		case *database.RegexMismatch:
			return model.RegexMismatchException{Message: err.Error()}
